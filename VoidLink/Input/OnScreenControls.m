...
static const CGFloat BUTTON_CENTER_INDICATOR_RADIUS = 3.5f;
static const CGFloat BUTTON_CENTER_INDICATOR_HIGHLIGHT_LINE_WIDTH = 0.0f;
static const CGFloat BUTTON_CENTER_INDICATOR_BASE_ALPHA = 0.04f;
static const CGFloat BUTTON_CENTER_INDICATOR_BASE_STROKE_ALPHA = 0.0f;
static const CGFloat BUTTON_CENTER_INDICATOR_BASE_LINE_WIDTH = 1.0f;
static NSString * const kIndicatorKeyDPadCenter = @"dpadCenter";
static NSString * const kIndicatorKeyABXYCenter = @"abxyCenter";
...
- (void) markGyroPauseButtonRegistered  { _controllerSupport.hasGyroPauseButton  = YES; }
- (void) clearMotionControlButtonRegistration { [_controllerSupport clearMotionControlButtonRegistration]; }
...
- (BOOL)handleTouchUpEvent:touches {
    ...
    for (UITouch* touch in touches) {
        ...
        if (touch == _aTouch) {
            ...
            _leftStick.opacity = _obscuredByAlpha ? OBSCURED_ALPHA : [_originalControllerLayerOpacityDict[_leftStick.name] floatValue];
            _leftStick.shadowOpacity = 0.0;
            _leftStick.opacity = _leftStickOpacity; // keep idle opacity rule
            [_controllerSupport updateLeftStick:_controller x:0 y:0];
            [_controllerSupport clearButtonFlag:_controller flags:LS_CLK_FLAG];
            ...
            _rightStick.opacity = _obscuredByAlpha ? OBSCURED_ALPHA : [_originalControllerLayerOpacityDict[_rightStick.name] floatValue];
...
- (BOOL)isObscuredByAlpha {
    return _obscuredByAlpha;
}

- (void)setObscuredByAlpha:(BOOL)enabled {
    _obscuredByAlpha = enabled;
    [CATransaction begin];
    [CATransaction setAnimationDuration:0.2];
    [CATransaction setAnimationTimingFunction:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
    for (CALayer* layer in self.OSCButtonLayers) {
        if (layer == self._leftStickBackground || layer == self._rightStickBackground) {
            layer.opacity = enabled ? OBSCURED_ALPHA : 1.0f;
        } else if (layer == self._leftStick || layer == self._rightStick) {
            layer.opacity = enabled ? OBSCURED_ALPHA : DEFAULT_STICK_OPACITY;
        } else {
...
- (void)ensureButtonCenterIndicators {
    ...
    if (!_buttonCenterIndicatorBases) {
        _buttonCenterIndicatorBases = [NSMutableDictionary dictionaryWithCapacity:5];
    }

    [self ensureIndicatorLayerForKey:kIndicatorKeyDPadCenter];
    [self ensureIndicatorLayerForKey:kIndicatorKeyABXYCenter];
}
...
- (void)ensureIndicatorLayerForKey:(NSString *)key {
    ...
    CAShapeLayer *highlight = [self createHighlightIndicatorLayer];
    CAShapeLayer *base = [self createBaseIndicatorLayer];
    if (!highlight || !base) {
        return;
    }
    [_view.layer addSublayer:base];
    [_view.layer addSublayer:highlight];
    _buttonCenterIndicators[key] = highlight;
    _buttonCenterIndicatorBases[key] = base;
}

- (CAShapeLayer *)createHighlightIndicatorLayer {
    ...
    layer.fillColor = [[UIColor colorWithWhite:1.0f alpha:0.1f] CGColor];
    layer.strokeColor = nil;
    layer.lineWidth = BUTTON_CENTER_INDICATOR_HIGHLIGHT_LINE_WIDTH;
    layer.contentsScale = [UIScreen mainScreen].scale;
    layer.hidden = YES;
    layer.opacity = 0.0f;
    layer.zPosition = 1001.0f;
    layer.compositingFilter = @"softLightBlendMode";
    return layer;
}

- (CAShapeLayer *)createBaseIndicatorLayer {
    CGFloat diameter = BUTTON_CENTER_INDICATOR_RADIUS * 2.0f;
    CGRect circleRect = CGRectMake(0.0f, 0.0f, diameter, diameter);
    ...
    layer.fillColor = [[UIColor colorWithWhite:1.0f alpha:BUTTON_CENTER_INDICATOR_BASE_ALPHA] CGColor];
    layer.strokeColor = [[UIColor colorWithWhite:0.0f alpha:BUTTON_CENTER_INDICATOR_BASE_STROKE_ALPHA] CGColor];
    layer.lineWidth = BUTTON_CENTER_INDICATOR_BASE_LINE_WIDTH;
    layer.contentsScale = [UIScreen mainScreen].scale;
    layer.hidden = YES;
    layer.opacity = 0.0f;
...
- (void)refreshButtonCenterIndicators {
    ...
    [self updateIndicatorForKey:kIndicatorKeyDPadCenter withLayer:self._dPadBackground ?: self._upButton];
    [self updateIndicatorForKey:kIndicatorKeyABXYCenter withLayer:self._aButton];
...
- (void)updateIndicatorForKey:(NSString *)key withLayer:(CALayer *)layer {
    ...
    CAShapeLayer *highlight = _buttonCenterIndicators[key];
    CAShapeLayer *base = _buttonCenterIndicatorBases[key];
    if (!highlight && !base) {
        return;
    }

    CGPoint position = CGPointZero;
    BOOL hasValidPosition = NO;

    if ([key isEqualToString:kIndicatorKeyDPadCenter]) {
        position = [self dPadIndicatorPosition];
        hasValidPosition = !isnan(position.x) && !isnan(position.y);
    } else if ([key isEqualToString:kIndicatorKeyABXYCenter]) {
        position = [self abxyIndicatorPosition];
        hasValidPosition = !isnan(position.x) && !isnan(position.y);
    } else if (layer) {
        CGPoint centerPoint = [self centerPointForLayer:layer];
        hasValidPosition = !isnan(centerPoint.x) && !isnan(centerPoint.y);
        if (hasValidPosition) {
...
- (CGPoint)dPadIndicatorPosition {
    CALayer *backgroundLayer = self._dPadBackground;
    if (backgroundLayer && !backgroundLayer.hidden) {
        CGPoint centerPoint = [self centerPointForLayer:backgroundLayer];
        if (!isnan(centerPoint.x) && !isnan(centerPoint.y)) {
            return centerPoint;
        }
    }

    CALayer *layers[] = { self._upButton, self._downButton, self._leftButton, self._rightButton };
    ...
    if (visibleCount > 0) {
        return CGPointMake(sumX / (CGFloat)visibleCount, sumY / (CGFloat)visibleCount);
    }

    return CGPointMake(D_PAD_CENTER_X, D_PAD_CENTER_Y);
}

- (CGPoint)abxyIndicatorPosition {
    CALayer *layers[] = { self._aButton, self._bButton, self._xButton, self._yButton };
    size_t layerCount = sizeof(layers) / sizeof(layers[0]);
    CGFloat sumX = 0.0f;
    CGFloat sumY = 0.0f;
    size_t visibleCount = 0;
    ...
    return CGPointMake(BUTTON_CENTER_X, BUTTON_CENTER_Y);
}
...
- (CALayer *)layerForIndicatorKey:(NSString *)key {
    ...
    if ([key isEqualToString:kIndicatorKeyDPadCenter]) {
        return self._dPadBackground ?: self._upButton;
    }
    if ([key isEqualToString:kIndicatorKeyABXYCenter]) {
...
- (BOOL)shouldDisplayIndicatorForKey:(NSString *)key {
    ...
    if ([key isEqualToString:kIndicatorKeyDPadCenter]) {
        CALayer *layers[] = { self._dPadBackground, self._upButton, self._downButton, self._leftButton, self._rightButton };
...
- (void)setButtonCenterIndicatorsHidden:(BOOL)hidden animated:(BOOL)animated {
    ...
    [_buttonCenterIndicators enumerateKeysAndObjectsUsingBlock:^(NSString *key, CAShapeLayer *highlight, BOOL *stop) {
        BOOL suppressed = ![self shouldDisplayIndicatorForKey:key];
        BOOL effectiveHidden = hidden || suppressed;
        highlight.hidden = effectiveHidden;
        highlight.opacity = effectiveHidden ? 0.0f : 1.0f;
        CAShapeLayer *base = _buttonCenterIndicatorBases[key];
        if (base) {
            base.hidden = effectiveHidden;
            base.opacity = effectiveHidden ? 0.0f : 1.0f;
        }
    }];
    [CATransaction commit];
}
...
- (void)dealloc {
    ...
    for (CAShapeLayer *base in _buttonCenterIndicatorBases.allValues) {
